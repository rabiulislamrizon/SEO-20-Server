...
async function run() {
    try {
        ...
        const employeeCollection = client.db('toyhouse').collection('employeeCollection');
        const projectCollection = client.db('toyhouse').collection('projectCollection');
        const subscriptionCollection = client.db('toyhouse').collection('subscriptionCollection');

        const ticketsAllCollection = client.db('toyhouse').collection('ticketsCollection');
        ...
        app.post('/add-ticket', async (req, res) => {
            ...
            const result = await ticketsAllCollection.insertOne(addticket);
            res.send(result);
        });
        app.get('/ticket', async (req, res) => {
            try {
                ...
                // Create a query object that is either empty or filters by email
                const query = email ? { customerEmail: email } : {};

                // Fetch tickets based on the query
                const cursor = ticketsAllCollection.find(query);
        ...
        app.get('/ticket/:id', async (req, res) => {
            ...
            const ticket = await ticketsAllCollection.findOne(query);
            res.send(ticket);
        ...
        app.put("/ticket/:id", async (req, res) => {
            ...
            const updatedDoc = {
                $set: {
                    ticketStatus: updateTicket.ticketStatus,
                    replies: updateTicket.replies,  // This will store the array of replies
                },
            };

            const result = await ticketsAllCollection.updateOne(
                filter,
                updatedDoc,
                options
            );
        ...
        app.post('/add-subscribtion', async (req, res) => {
            const addsubscribtion = req.body;
            const result = await subscriptionCollection.insertOne(addsubscribtion);
            res.send(result);
        });

        app.get('/get-subscribtion', async (req, res) => {
            const query = {};
            const cursor = subscriptionCollection.find(query);
            const getsubscribtion = await cursor.toArray();
            res.send(getsubscribtion);
        });


        app.get('/get-subscribtion/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: ObjectId(id) };
            const subscribtion = await subscriptionCollection.findOne(query);
        ...
        app.post('/add-employee', async (req, res) => {
            ...
            const result = await employeeCollection.insertOne(addEmployee);
            res.send(result);
        });
        ...
        app.get('/employees', async (req, res) => {
            ...
            const employess = await cursor.toArray();
            res.send(employess);
        });
        ...
        app.get('/employees/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: ObjectId(id) };
            const employes = await employeeCollection.findOne(query);
            res.send(employes);
        ...
        app.put("/upadete-employee/:id", async (req, res) => {
            const id = req.params.id;
            const employesEdit = req.body;
            const filter = { _id: new ObjectId(id) };
            const options = { upsert: true };
            const updatedDoc = {
                $set: {
                    EmployeeName: employesEdit.EmployeeName,
                    EmployeeEmail: employesEdit.EmployeeEmail,
                    EmployeeRole: employesEdit.EmployeeRole,
        ...
        /* Project Section Add Start */


        app.post('/add-project', async (req, res) => {
            const addProject = req.body;
            const result = await projectCollection.insertOne(addProject);
            res.send(result);
        });
        ...
        app.put("/upadete-project/:id", async (req, res) => {
            const id = req.params.id;
            ...
            const updatedDoc = {
                $set: {
                    projectDate: projectEdit.projectDate,
                    projectName: projectEdit.projectName,
                    projectBudget: projectEdit.projectBudget,
                    advancePayment: projectEdit.advancePayment,
                    duePayment: projectEdit.duePayment,
                    projectStatus: projectEdit.projectStatus,
        ...
        app.put("/update-counters/:id", async (req, res) => {
            ...
            const updatedDoc = {
                $set: {
                    ...
                    teamNumber: counterSection.teamNumber,
                    teamText: counterSection.teamText,
        ...
        app.put("/update-about/:id", async (req, res) => {
            ...
            const updatedDoc = {
                $set: {
                    ...
                    pointOne: aboutSection.pointOne,
                    pointTwo: aboutSection.pointTwo,
                    pointThree: aboutSection.pointThree,
                },
            };
...
app.get('/', (req, res) => {
    res.send('Good Job, SEO-20 Server is Working Fine');
});

app.listen(port, () => {
    console.log('Good Job, SEO-20 Server is Live Now', port);
});